...
{
    public async Task InvokeAsync(HttpContext context, JwtTokenService jwtTokenService, JwtSecurityTokenHandler jwtSecurityTokenHandler)
    {
        try 
        {
        ...
        {
            // Ocurrio un error al revisar el token bro
        }
        await next(context);
    }
}
